...
const display_search_value = document.getElementsByClassName("text");
...
export function setValue(index){
  ...
  const h1 = parseInt(styleOfElement.height, 10);
  front_color_bar[index].textContent = h1;
}

export function getInputValue(){
    const styleOfInput = document.getElementsById("SearchInput").textContent;
    display_search_value.textContent = styleOfInput;
}
...
export function swapBars(index1, index2) {
  const styleOfElement1 = getBarStyle(index1),
   ...
   for (let j = 0; j < 4; j++) {
    ...
    front_color_bar[index1].textContent = h1
    front_color_bar[index2].textContent = h2
